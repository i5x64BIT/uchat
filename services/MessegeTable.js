...
import {asyncMessegeHandler} from "./AsyncHandler.js";
...
export async function queryGetMessegesByUserID(userID){
    try{
        ...
        asyncMessegeHandler(e);
    }
}
//for conversations
export async function queryGetMessegesBySenderToReciver(senderID, receiverID){
    try{
        const db = await connectDB();
        const querySelect = `
...
export async function queryMessegeCreate(messege){
    try{
        const db = await connectDB();
        let queryInsert = `
